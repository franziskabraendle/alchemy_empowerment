...
def create_gametreetable_file(time=None, prediction_model=0, n_elements=None, game_version='alchemy2', split_version='data', vector_version='crawl300'):
...
def append_gametreetable_file(*data, time=None, prediction_model=0, first_line=False, game_version='alchemy2', split_version='data', vector_version='crawl300'):
    ...
    for line in data_new:
        if prediction_model == 0:
            ...
            with open('empowermentexploration/data/gametree/{}/{}EmpPredTable-{}-{}.csv'.format(time, game_version, split_version, vector_version), 'a+', newline='') as outfile:
                writer = csv.writer(outfile)
                writer.writerow(line)
