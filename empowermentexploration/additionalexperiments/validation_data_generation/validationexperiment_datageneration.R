...
# write.csv(sampled_df, "predSuccess_balanced_with_true_random_combinations.csv", row.names = FALSE)


#filtering out which 4 elements are used
combination_selection<-read.csv("predSuccess_balanced_with_true_random_combinations.csv")
...
write.csv(df_finalvalues, "prediction_selection_dataframe_allinfo_truebalanced_AAA.csv", row.names = FALSE)

#as json for experiment (also emp file)
jsonfile = toJSON(test)
write(jsonfile, "df_prediction_2.json")
...
write(jsonfile2, "df_empowerment.json")
