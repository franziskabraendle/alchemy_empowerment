...
{	
	...
	if(roundcounter == 0){
		...
		change("counter", "Rounds left: "+roundcounter)
		isexp=1;
...
function mysubmit()
{
  ...
  if (turkid=="WorkerID")
  	...
  	}else
  	{
		...
		var validation=2021;
		myDataRef.push({'collect': collect, 'elements': elements, 'randomcombinations': randomcombinations, 'currentcombinationstring' : currentcombinationstring, 'linkanswers': linkanswers, 'elementoptionsshuffeld': elementoptionsshuffeld, 'elementanswers': elementanswers, 'randomempelement': randomempelement, 'empowermentanswers': empowermentanswers, 'age': age, 'gender': gender, 'playedLA': playedLA, 'turkid': turkid, 'validation' : validation});
  	}
}
...
var currentprediction=["questionmark", "questionmark", "redquestionmark"];
var currentcombinationstring=[];
var currentcombination;
var randomcombinations =[]
var predictions = df_prediction;
var predictioncounter = 20;
var linkpredTouched = false;
var allpredictions = [];

function drawlinkprediction()
{
	...
	linkpredTouched = false
	predictioncounter --;
	currentcombination = randomNum(0,1000);
	while(allpredictions.includes(currentcombination)){
		currentcombination = randomNum(0,1000);
	}
	allpredictions.push(currentcombination);
	
	currentprediction=[predictions["first_string"][currentcombination], predictions["second_string"][currentcombination], "redquestionmark"]
	currentcombinationstring.push([predictions["first_string"][currentcombination], predictions["second_string"][currentcombination]]);
	var linkprops='<img src="inventpic/'+currentprediction[0]+'.png" width="50px" height="50px"  title = '+currentprediction[0]+'>&nbsp&nbsp&nbsp'+
			  '<img src="proppics/plus.png" width="35px" height="35px">&nbsp&nbsp&nbsp'+
	          '<img src="inventpic/'+currentprediction[1]+'.png" width="50px" height="50px" title = '+currentprediction[1]+'>&nbsp&nbsp&nbsp'+
	   		  '<img src="proppics/equals.png" width="35px" height="35px">&nbsp&nbsp&nbsp'+
	   		  '<img src="proppics/'+currentprediction[2]+'.png" width="35px" height="50px">&nbsp&nbsp&nbsp';
	change('linkexp', linkprops);
	randomcombinations.push(currentcombination)
	}
}
drawlinkprediction();
...
var linkanswers =[];
var elementoptionsshuffeld = [];
var fourelementsstring=[];

var linkpredInput = document.getElementById("linkpredSlider");
var linkpredOutput = document.getElementById("linkpredValue");
linkpredOutput.innerHTML = linkpredInput.value;

linkpredInput.oninput = function() {
	linkpredOutput.innerHTML = this.value;
    linkpredTouched = true;
  }

function linksubmit()
{
	if (linkpredTouched == false){
		alert("Please move the slider before submitting.");
	}
	else{
		clickStart("linkquestion", "elementquestion")
		document.getElementById("linkpredparagraph").style.display ='none' ;
		document.getElementById("fourelements").style.display ='block' ;
	

		fourelementsstring=[predictions["E1_string"][currentcombination],predictions["E2_string"][currentcombination],predictions["E3_string"][currentcombination],predictions["E4_string"][currentcombination],];
		shuffle(fourelementsstring)
		var fourelems = [];
		for(var i=0; i<fourelementsstring.length; i++) 
		{
			fourelems=fourelems+'<img src="inventpic/'+fourelementsstring[i]+'.png" width="50" height="50"';
			fourelems=fourelems+'title= "'+fourelementsstring[i]+'"';
			fourelems=fourelems+' onclick="elementsubmit('+i+')">';
		}

		linkanswers.push(document.getElementById("linkpredSlider").value);
		document.getElementById("linkpredSlider").value = 4;
		document.getElementById("linkpredValue").innerHTML = document.getElementById("linkpredSlider").value;
		elementoptionsshuffeld.push(fourelementsstring);
		change("fourelements",fourelems);
	}
}


var elementanswers = [];
function elementsubmit(value)
{
	elementanswers.push(fourelementsstring[value])
	drawlinkprediction();
	clickStart("elementquestion", "linkquestion")
	document.getElementById("linkpredparagraph").style.display ='inline' ;
	document.getElementById("fourelements").style.display ='none' ;
}

var empquestioncounter = 20;
var currentempelement;
var empowerment = df_empowerment;
var empowermentanswers = [];
var randomempelement = [];

var emppredInput = document.getElementById("emppredSlider");
var emppredOutput = document.getElementById("emppredValue");
emppredOutput.innerHTML = document.getElementById("emppredSlider").value;

var emppredTouched = false
emppredInput.oninput = function() {
	emppredOutput.innerHTML = this.value;
    emppredTouched = true;
}


var twentyEmpElements =[];

for (var j = 0; j <5; j++){
	currentempelement = randomNum(0, empowerment["element_string"].length);
	while(twentyEmpElements.includes(empowerment["element_string"][currentempelement]) || empowerment["emp_value_predicted"][currentempelement] > 3){
		currentempelement = randomNum(0, empowerment["element_string"].length);
	}
	twentyEmpElements.push(empowerment["element_string"][currentempelement])

}
for (var j = 0; j <5; j++){
	currentempelement = randomNum(0, empowerment["element_string"].length);
	while(twentyEmpElements.includes(empowerment["element_string"][currentempelement]) || empowerment["emp_value_predicted"][currentempelement] > 14 || empowerment["emp_value_predicted"][currentempelement] < 4){
		currentempelement = randomNum(0, empowerment["element_string"].length);
	}
	twentyEmpElements.push(empowerment["element_string"][currentempelement])
...
for (var j = 0; j <5; j++){
	currentempelement = randomNum(0, empowerment["element_string"].length);
	while(twentyEmpElements.includes(empowerment["element_string"][currentempelement]) || empowerment["emp_value_predicted"][currentempelement] > 39 || empowerment["emp_value_predicted"][currentempelement] < 15){
		currentempelement = randomNum(0, empowerment["element_string"].length);
	}
	twentyEmpElements.push(empowerment["element_string"][currentempelement])
...
for (var j = 0; j <5; j++){
	currentempelement = randomNum(0, empowerment["element_string"].length);
	while(twentyEmpElements.includes(empowerment["element_string"][currentempelement]) || empowerment["emp_value_predicted"][currentempelement] < 40){
		currentempelement = randomNum(0, empowerment["element_string"].length);
	}
	twentyEmpElements.push(empowerment["element_string"][currentempelement])

}
shuffle(twentyEmpElements);


function drawempquestion()
{
	if(empquestioncounter == 0){
		clickStart("page6", "page7")
		
	}
	else{
		empquestioncounter --;
		emppredTouched = false
		document.getElementById("emppredValue").value="";
		randomempelement.push(twentyEmpElements[empquestioncounter]);
		change("empelement",'<img src="inventpic/'+twentyEmpElements[empquestioncounter]+'.png" width="50px" height="50px" title = '+twentyEmpElements[empquestioncounter] +'>');
...
function submitemp(){
	...
	if(emppredTouched == false)
	{
  		alert("Please move the slider before submitting.");
	}
	else{
	empowermentanswers.push(document.getElementById("emppredSlider").value)
	document.getElementById("emppredSlider").value = 4;
	document.getElementById("emppredValue").innerHTML = document.getElementById("emppredSlider").value;
	drawempquestion();
	}
}
