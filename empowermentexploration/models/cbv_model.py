...
class CBVModel():
    ...
    def update_model_specifics(self, combination, results, inventory, step):
        ...
        if len(results[1]) == 0:
            self.values_elements[combination[0]] -= 1
            self.values_elements[combination[1]] -= 1
        else: 
            self.values_elements[combination[0]] += len(results[1])
            self.values_elements[combination[1]] += len(results[1])
