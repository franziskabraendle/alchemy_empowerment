...
class CrossValidation():
    def __init__(self, k, time, game_version='alchemy2', 
                 link_prediction=True, epochs=1, batch_size=32, steps_per_epoch=None, 
        ...
        Args:
            ...
            link_prediction (bool, optional): States for what model the data is generate, 
            ...
            oversampling (float, optional): Desired fraction to what an underrepresented class is to by oversampled. 
                        Is only applied, if link_prediction is True.
    ...
    def run_cross_validation(self):
        ...
        log.create_gametreetable_file(time=self.time, link_prediction=self.link_prediction, n_elements=self.n_elements, 
        ...
        if self.link_prediction is True:
        ...
        if self.exclude_elements_test is True:
            ...
            for idx, test_group in enumerate(element_groups):
                ...
                if self.exclude_elements_val is True:
                    ...
                    data = PreparedDataset(data_table, game_version=self.game_version, link_prediction=self.link_prediction,  
            ...
            for train_index, test_index in kf.split(data_table):
                # get data
                data = PreparedDataset((data_table.iloc[train_index], data_table.iloc[test_index]), game_version=self.game_version, link_prediction=self.link_prediction, 
    ...
    def run_cross_validation_round(self, data):
        ...
        # write to file
        log.append_gametreetable_file(data.idx_test, predictions, time=self.time, link_prediction=self.link_prediction, 
                                      game_version=self.game_version, split_version=self.split_version, vector_version=self.vector_version)
