...
la2imushum<-read.csv("..\\..\\..\\..\\empowermentexploration\\resources\\playerdata\\data\\dataImUs\\20211103-1125-alchemy2-immediateusageWithoutLog-Memory-human.csv")
la2imusran<-read.csv("..\\..\\..\\..\\empowermentexploration\\resources\\playerdata\\data\\dataImUs\\20211103-1125-alchemy2-immediateusageWithoutLog-Memory-random.csv")


humdf<-data.frame(emp = (c(la2imushum$x1)),
                   prob = c(la2imushum$y))

randf<-data.frame(emp = (c(la2imusran$x1)),
                  prob = c(la2imusran$y))

humdf$prob <- as.numeric(as.character(humdf$prob))
randf$prob <- as.numeric(as.character(randf$prob))
humdf$emp_scaled <- scale(as.numeric(as.character(humdf$emp)))
randf$emp_scaled <- scale(as.numeric(as.character(randf$emp)))

kendalcorhum<- cor.test(humdf$emp, humdf$prob, method='kendal')
kendalcorhum
kendalcorran<- cor.test(randf$emp, randf$prob, method='kendal')
kendalcorran
...
humanci <- kendall.ci(x=humdf$emp, y=humdf$prob, alpha=0.05, type="t", bootstrap=F, B=1000, example=F)
randomci <- kendall.ci(x=randf$emp, y=randf$prob, alpha=0.05, type="t", bootstrap=F, B=1000, example=F) 
